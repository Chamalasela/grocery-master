(() => {
  ...
  const defaults = () => ({version: 1, week: weekOf(), items: [
    ['Minced chicken', '250 g'], ['Greek yogurt', '1 tub'], ['Eggs', '6'],
    ['Avocados', '2'], ['Bananas', '6'], ['Baby spinach', '1 bag'],
    ['Oat milk', '1 L'], ['Sourdough bread', '1 loaf']
  ].map(([name, quantity]) => ({id: makeId(), name, quantity, done: false}))});
  ...
  function render() {
    ...
    visible.forEach(item => {
      ...
      const name = document.createElement('span'); name.className = 'item-label'; name.textContent = item.name;
      const quantity = document.createElement('span'); quantity.className = 'quantity-badge'; quantity.textContent = item.quantity;
      ...
      edit.addEventListener('click', () => { editingId = item.id; $('edit-name').value = item.name; $('edit-quantity').value = item.quantity; $('edit-dialog').showModal(); });
      ...
      actions.append(edit, remove); row.append(check, name, quantity, actions); list.append(row);
    });
  ...
  $('add-form').addEventListener('submit', event => {
    ...
    rollover(); state.items.push({id: makeId(), name, quantity, done: false}); filter = 'all'; save(); render(); $('add-form').reset(); $('item-name').focus(); announce(`${name} added to your weekly list.`);
  });
  $('edit-form').addEventListener('submit', event => {
    ...
    if (item) Object.assign(item, {name, quantity});
